...
def calculate_average_slowdown(compute_conn, avg_memcpy_tables, memcpy_tables):
    ...
    for avg_table, true_table in zip(avg_memcpy_tables, memcpy_tables):
        ...
        SELECT deviceId, duration, overlap_ratio 
        FROM {true_table};
        ''', compute_conn)

        # Ensure we match by deviceId
        slowdown_df = pd.merge(df_memcpy, avg_memcpy_df, on=['deviceId'])
...
def main():
    ...
    avg_slowdown_df.to_csv('./res/average_slowdown_per_device.csv', index=False)
